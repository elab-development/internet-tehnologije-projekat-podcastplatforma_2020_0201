...
const FindPodcast: React.FC<FindPodcastProps> = ({
  ...
  }, [token, fetchPodcasts, getUserLikes, fetchAllComments]);
      ...
      <div className="find-podcast">
        <div className="find-content">
          ...
          {filteredPodcasts?.length === 0 && (
            <p>No podcasts available for the selected category.</p>
          )}
          <div className="video-list">
            {filteredPodcasts?.map((podcast) => (
              <div key={podcast.id} className="video-item">
                <video
                  controls
                  src={`http://localhost:8000/storage/${podcast.video_url}`}
                ...
                {selectedPodcast === podcast.id && (
                  <div>
                    ...
                    />
                    <button onClick={() => handleCommentSubmit(podcast.id)}>
                      Submit Comment
                    </button>
                  </div>
                )}
                <div className="comments-list">
                  {comments
                    .filter((comment) => comment.podcast_id === podcast.id)
                    .map((comment, index) => (
...
const mapDispatchToProps = (dispatch: any) => ({
  likePodcastRequest: (podcastId: number, token: string) =>
    dispatch(likePodcastRequest(podcastId, token)),
  commentOnVideo: (podcast_id: number, comment: string, token: string) =>
    dispatch(commentOnVideo({ podcast_id, comment }, token)),
  fetchPodcasts: (token: string) => dispatch(fetchPodcastsRequest(token)),
  getUserLikes: (token: string) =>
    dispatch({ type: "GET_LIKES", payload: token }),
