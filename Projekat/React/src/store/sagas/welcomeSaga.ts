...
function* changePasswordSaga(action: any) {
  try {
    const { email, name, new_password, new_password_confirmation } =
      action.payload;
    const response: AxiosResponse<PasswordChangeResponse> = yield call(
      axios.put,
      `${API_URL}change-password`,
      { email, name, new_password, new_password_confirmation }
    );
    if (response.status === 200) {
...
function* registerSaga(action: any) {
  try {
    const { email, name, password, password_confirmation, role } =
      action.payload;
    ...
    if (response.status === 201) {
      ...
      if (message) {
        yield put(
          setRegisterResponse(
            "Morate sačekati da naši administratori odobre registraciju."
          )
        );
      } else if (user && token) {
    ...
    if (axios.isAxiosError(error) && error.response) {
      yield put(setRegisterResponse("RGISTRACIJA NEUSPESNA"));
    } else {
